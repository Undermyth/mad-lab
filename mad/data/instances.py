...
def generate_distract_fuzzy_in_context_recall_instance(
    vocab_size: int = 16,
    seq_len: int = 128,
    k_motif_size: int = 3,
    v_motif_size: int = 3,
    ...
    *args, **kwargs
    ...
    # generate key-value probe:
    k_len_list = list(keys.keys())
    if is_training:
        k_len_list.remove(1)
    k_probe_size = rng.choice(k_len_list) if is_training else k_motif_size
    v_probe_size = rng.choice(list(values.keys()))
    k_probe = tuple(rng.choice(keys[k_probe_size]))
    v_probe = tuple(rng.choice(values[v_probe_size]))
    kv_probe_size = k_probe_size + v_probe_size
    probe_idx = rng.choice(seq_len - kv_probe_size - 3 * (k_motif_size + v_motif_size)) # make sure we add key-value pair to the non-probe part of the sequence
    probe_added = False # flag indicating whether key-value probe has been added to the sequence    

    # generate distraction probe:
    diff_idx = rng.choice(range(k_probe_size))
    diff_token = rng.choice(key_vocab)
    k_distract = list(k_probe)
    k_distract[diff_idx] = diff_token
    k_distract = tuple(k_distract)
    v_distract = tuple(rng.choice(values[v_probe_size]))
    distract_idx = rng.choice(range(probe_idx + kv_probe_size, seq_len - kv_probe_size - 2 * (k_motif_size + v_motif_size)))
    distract_added = False
    ...
    while len(inputs) < seq_len - kv_probe_size - (k_motif_size + v_motif_size): 
        ...
        if len(inputs) >= distract_idx and not distract_added:
            inputs.extend(k_distract)
            inputs.extend(v_distract)
            targets.extend(tuple([target_ignore_idx]*(k_probe_size+len(v_probe))))
            train_targets.extend(tuple([target_ignore_idx]*k_probe_size))
            train_targets.extend(v_distract)

            kv_map[k_probe_size][k_distract] = v_distract
            keys_presented[k_distract] = v_distract
            distract_added = True
            continue
    ...
    # print('probe: ', k_probe, v_probe)
    # print('distract: ', k_distract, v_distract)
    assert distract_added and probe_added, f"distract added: {distract_added}, probe added: {probe_added}"
